...
class BaseModel {
    ...
    openConnection() {
        if (!configurations_1.CONFIGURATIONS.connection) {
            return new connection_1.Connection().createConnection().then(res => {
                console.log('-----------------------------------------------------------');
                console.log('Db Connection is created (' + new Date() + ')');
                console.log('-----------------------------------------------------------');
                this.connection = res;
                configurations_1.CONFIGURATIONS.connection = res;
            });
        }
        else {
    ...
    closeConnection() {
        ...
        configurations_1.CONFIGURATIONS.connection = null;
    }
    /**
    ...
    */
    ...
    count(condition, includes, xtra) {
        return this.sequelizeModel.count(this.sequelizeQueryBuilder(null, condition, includes, null, null, xtra));
    }
    ...
    */
    ...
    deleteAllByConditions(conditions, xtra) {
        return this.findAllByConditions(['id'], conditions, null, null, xtra).then(res => {
            if (res && res.length) {
                return bluebird_1.Promise.each(res, (el) => {
    ...
    * @param condition any
    */
    sequelizeQueryBuilder(attributes, condition, include, order, options, xtra) {
        let obj = {};
